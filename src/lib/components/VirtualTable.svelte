...
<div class="h-full w-full flex flex-col bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
  ...
  <div 
    class="flex-1 overflow-auto bg-slate-900/60" 
    bind:this={parentEl}
  >
    <div class="min-w-max relative flex flex-col min-h-full" style="height: {appState.filteredRows.length > 0 ? totalSize : '100%'}px;">
      <!-- Table Header (Sticky) -->
      <div 
        class="sticky top-0 z-10 font-bold text-xs uppercase tracking-wider bg-slate-950 text-slate-200 border-b border-slate-800 select-none"
        style="height: 40px;"
      >
        <div
